...
export function createBloom(renderer) {
  ...
  // Понижено по запросу Олега (перф): свечение мягкое и размытое по природе — меньшее
  // разрешение делает гало чуть шире/мягче (для bloom это незаметно или даже приятнее),
  // зато стоимость падает ~вдвое (стоимость ∝ SCALE²). Слой мглы НЕ трогаем — только bloom.
  const SCALE = LOW_END ? 0.14 : 0.18;
