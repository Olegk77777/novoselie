...
void main(){
  ...
  // нижняя маска: 1 у самого низа, 0 к ~58% высоты; степень 1.3 — гуще тело, язычки достают
  // выше к открытым кромкам пола (запрос Олега «туман гуще, заползает внутрь»).
  float bottomMask = smoothstep(0.58, 0.0, uv.y);
  bottomMask = pow(bottomMask, 1.3);

  // РАННИЙ ВЫХОД: выше ~58% и при полной расчистке — чистая прозрачность.
  // Верхние ~42% кадра почти бесплатны и не трогают освещённую комнату.
  ...
  // в «языки»: степень + порог оставляют тяжи (порог ниже/шире — язычки крупнее, гуще)
  float tongues = pow(clamp(body, 0.0, 1.0), 1.5);
  tongues = smoothstep(0.14, 0.74, tongues);
  ...
  // итоговая альфа: только низ, гуще прежнего, уходит с расчисткой
  float alpha = bottomMask * tongues * sideMask * 0.52 * (1.0 - clear);
  ...
  gl_FragColor = vec4(veilCol, clamp(alpha, 0.0, 0.55)); // прямой (не premultiplied) alpha
}
`;
