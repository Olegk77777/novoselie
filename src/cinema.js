...
export function createCinema({ camera, targetY, getBaseZoom, cards, osdLabel, osdDate, getFocusAnchors, filters }) {
  ...
  make('vhs-bleed');   // хром-бахрома у краёв (усиленная аберрация)
  make('vhs-scan');    // слабые горизонтальные VHS-строки (мягче, чем у кинескопа)
  make('vhs-top');     // аналоговые искажения у ВЕРХНЕЙ кромки (снег + рваные строки, дрожат на месте)
  make('vhs-bottom');  // аналоговые искажения у НИЖНЕЙ кромки (head-switch: рваная зона помех)
