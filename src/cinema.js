...
export function createCinema({ camera, targetY, getBaseZoom, cards, osdLabel, osdDate, getFocusAnchors, filters }) {
  ...
  // СЛОИ ФИЛЬТРА «КАССЕТА» (VHS). Видимы только по body.cine-filter-vhs (см. index.html CSS).
  // Домашняя видеозапись 90-х: выцветшая лента, мягкая хром-аберрация по краям, лёгкие
  // tracking-полосы. Тоже чистые CSS-оверлеи (выцвет — backdrop-filter на десктопе).
  make('vhs-grade');   // выцвет ленты: десатурация (десктоп) + тёплый молочный грейд
  make('vhs-scan');    // слабые горизонтальные VHS-строки (мягче, чем у кинескопа)
  make('vhs-top');     // снег/помехи у ВЕРХНЕЙ кромки (дрожат на месте)
  make('vhs-bottom');  // снег/помехи у НИЖНЕЙ кромки (поверх «съезда» кадра — head-switch)
  // Настоящая хром-аберрация (R/B расходятся от КРАЁВ объектов) + «съезд» нижней полосы —
  // это SVG-фильтр на самом <canvas> (CSS body...vhs > canvas), не оверлей. Defs — в index.html.

  // СЛОИ ФИЛЬТРА «8 ММ» (Супер-8). Видимы только по body.cine-filter-s8 (см. index.html CSS).
  // Домашнее кино 60–80-х: тёплый янтарный выцвет, оранжевые засветки-light leaks по краям,
  // мягкая тёплая виньетка, крупное зерно, царапины. Дрожание кадра (gate weave) и плёночное
  // мерцание яркости — анимация transform/brightness на самом <canvas> (composite-only, дёшево).
  make('s8-grade');     // тёплый янтарный грейд + выцвет (десатур./сепия на десктопе)
