...
export function createPlacement({ scene, camera, canvas, floor, cols, rows, wallSurfaces = [], onStateChange, onComfortChange }) {
  ...
  function findDefaultWallSpot() {
    for (const s of wallSurfaces) {
      for (let h = s.heightMax - wallHalf.h; h >= wallHalf.h; h -= 0.5) {
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    if (ghost) {
      ...
      if (def.placement === 'wall') {
        ...
        place();
      } else if (currentAnchor && isFree(currentAnchor, rotationSteps)) {
        place();
      }
    } else {
      tryPickup(e);
